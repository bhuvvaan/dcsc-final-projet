...
logging.info(f"Inside video reader")
...
results_bucket_name = "ppt-result"
...
conn = psycopg2.connect(
    host="my-postgresql",   # or the IP of your service if not using port-forwarding
    port=5432,
    user="postgres",
...
def update_postgres(id):
    ...
    #object_name = "15939_presentation.pptx"

    try:
        ppt_url = MINIO_CLIENT.presigned_get_object(
            bucket_name, ppt_name
        )
        print("Presigned URL:", ppt_url)
    except Exception as e:
    ...
    CREATE TABLE IF NOT EXISTS video_details (
        video_id SERIAL PRIMARY KEY,         -- Automatically increments the ID
        video_link_minio TEXT NOT NULL,      -- URL to the video in MinIO
        ppt_link TEXT                        -- URL to the PPT in MinIO
    );
    '''

    cur.execute(create_table_query)
    print("Table 'video_details' created successfully (or already exists).")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process a video and create a PowerPoint from extracted frames.")
    parser.add_argument("--video_path", required=True, help="Path to the input video file.")
    parser.add_argument("--output_dir", required=True, help="Directory to save the extracted frames.")
    parser.add_argument("--interval", type=int, default=3, help="Interval (in seconds) to capture frames.")
    parser.add_argument("--id", type=int, required=True, help="Unique ID (retrieved from Redis).")
    ...
    image_folder = output_dir
    output_ppt = f"{output_dir}/images_presentation_{id}.pptx"  # Create output PPT filename
